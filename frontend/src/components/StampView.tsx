...
function CommittedRecord({ submission }: { submission: SubmissionResponse }) {
  ...
  return (
    <div className={`status-box ${submission.status}`} style={{ marginBottom: 16 }}>
      ...
      <div className="status-meta">
        ...
        <div className="status-row" style={{ alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button className="btn btn-secondary" style={{ padding: "4px 12px", fontSize: "0.75rem" }} onClick={copyId}>
                {copied ? "✓ Copied" : "Copy ID"}
              </button>
              <button className="btn btn-secondary" style={{ padding: "4px 12px", fontSize: "0.75rem" }} onClick={downloadTxt}>
                Save as .txt
              </button>
            </div>
          </div>
        ...
        <div className="status-row">
          ...
          <span className="status-val" style={{ wordBreak: "break-all" }}>{submission.fileHash}</span>
        </div>
        ...
        {submission.transactionHash && (
          <div className="status-row">
            ...
            <span className="status-val" style={{ wordBreak: "break-all" }}>{submission.transactionHash}</span>
          </div>
        )}
...
export function StampView() {
  ...
  const [submittedSnapshot, setSubmittedSnapshot] = useState<SubmissionResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [dragging, setDragging] = useState(false);

  // Past submissions shown above the form
  const [history, setHistory] = useState<{ id: string; snapshot: SubmissionResponse }[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await submitHash(fileHash);
  ...
  const handleTimestampAnother = () => {
    ...
    if (submissionId && submittedSnapshot) {
      setHistory(h => [...h, { id: submissionId, snapshot: submittedSnapshot }]);
    }
    setFile(null);
    setFileHash(null);
    setSubmissionId(null);
        ...
        <div className="card">
          ...
          {!file ? (
            ...
            >
              ...
              <p className="dropzone-text"><strong>Drop your document here</strong> or click to browse</p>
              <p className="dropzone-hint">Any file type · Hash computed locally · Document never uploaded</p>
            </label>
          ) : (
            <>
              <div className="file-info">
                ...
                <button
                  ...
                  onClick={() => { setFile(null); setFileHash(null); setError(null); }}
                >
                  Change
                </button>
              </div>
              ...
              {fileHash && (
                <>
                  <div className="hash-display">
                    <div className="hash-label">keccak256 hash</div>
                  ...
                  <p className="text-muted" style={{ marginBottom: 16 }}>
                    Only the hash above will be sent to the server. Your document stays on your machine
                    and will be included in the next batch commit to the blockchain.
                  </p>

                  {error && (
                    <div className="status-box failed" style={{ marginBottom: 16 }}>
                      <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", color: "var(--error)" }}>{error}</p>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    disabled={submitting}
                  >
                    {submitting ? <><div className="spinner" /> Submitting…</> : "Timestamp this file →"}
                  </button>
                </>
              )}
            </>
